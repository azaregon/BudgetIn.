...
//    @Published var color
    ...
    @Published var dePersonalData = personalData2(iincome: 0,bbalance:0,point:0)
    ...
    @Published var perCategoryUserLimit = [
        "Needs" : perCategoryLimitData(name: "Needs" , percentage: 0, limit: 1, spentNow: 0),
        "Bills" : perCategoryLimitData(name: "Bills" , percentage: 0, limit: 1, spentNow: 0),
        "Wants" : perCategoryLimitData(name: "Wants" , percentage: 0, limit: 1, spentNow: 0),
        "Save"  : perCategoryLimitData(name: "Save"  , percentage: 0, limit: 1, spentNow: 0),
    ]
    
    @Published var lastPercentChangeDate = Date()
    
    //Sub Category List Data
    var subCategoryList:[String:[subCategoryListData]] = [  //NOT TO REMOVE
        "Needs": [
...
//            subCategoryListData(icon: "questionmark.app", name: "Others"),
    ...
    @Published var history:[String:[History]] = [
        "Needs" : [
...
//
//    ]
    @Published var subCategories:[String:[subCatData]] = [
        "Needs" : [
...
//    }
    ...
    func userCategoryIncreaseSpent(category:String) {
        ...
        for theOne in history[category]!.filter({$0.date.formatted(.dateTime.month().year()) == Date().formatted(.dateTime.month().year()) }) {
...
//    }
    ...
    func perSubCategorySpentIncrease(category:String){
        subCategories[category]!.enumerated().forEach { i,data in
            ...
            history[category]!.filter{x in x.subCat == data.name}.filter({$0.date.formatted(.dateTime.month().year()) == Date().formatted(.dateTime.month().year()) }).forEach { y in
                spentTotal += y.price
            }
...
//    }
    ...
    func calculateBalance() {
        ...
        for (key,category) in history {
            ...
            userCategoryIncreaseSpent(category: key)
            perSubCategorySpentIncrease(category: key)
            for theOne in category.filter({$0.date.formatted(.dateTime.month().year()) == Date().formatted(.dateTime.month().year()) }) {
                substractionTotal -= theOne.price
